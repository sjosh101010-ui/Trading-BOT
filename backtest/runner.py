        ...
        if pending_entry is not None:
            ...
            if filled:
                ...
                if entry.get("from_hc"):
                    ...
                    sl_pips = abs(fill_price - sl_price) / ps
                else:
                ...
                lot = compute_lot_size_sim(symbol, balance, sl_pips, entry["confidence"], price=fill_price)
                if lot <= 0:
                    ...
                    if len(open_positions[symbol]) >= MAX_STRATEGIES_PER_SYMBOL:
                        ...
                        same_direction_count = sum(
                            1 for p in open_positions[symbol].values()
                            if p["direction"] == entry["direction"]
                        )
...
def _check_sl_tp(current_row, open_positions, trades, current_time) -> float:
    ...
    for sym, strategies in list(open_positions.items()):
        for strategy_name, pos in list(strategies.items()):
            ...
            if hit_sl:
                exit_price = (
                    sl - spread_pts / 2 - slippage_pts if direction == "BUY"
                    else sl + spread_pts / 2 + slippage_pts
                )
                ...
                exit_price = (
                    tp + spread_pts / 2 + slippage_pts if direction == "BUY"
                    else tp - spread_pts / 2 - slippage_pts
                )
                pnl = (exit_price - entry) * lot * csize
                if direction == "SELL":
                    pnl = -pnl
...
def run_backtest(
    ...
    **kwargs,
    ...
    trades, metrics = backtest(
        df, symbol=symbol, trade_date=trade_date,
