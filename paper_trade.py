#!/usr/bin/env python3
"""
M5 Paper Trader — SimpleFX Live Execution
Yahoo M5 data for signals, SimpleFX API for execution.
Monitored via SimpleFX mobile app.
"""
...
def open_trade(symbol, direction, pa_score, adx_val, df, candle_time):
    ...
    be_trigger = (entry_price + RAPID_BE_ATR_MULT * atr if direction == "BUY" else entry_price - RAPID_BE_ATR_MULT * atr) if RAPID_BE_ATR_MULT > 0 else None
    ...
    if result.get("success"):
        ...
        logging.info(f"[{symbol}] ENTRY: {direction} {lot}L @ via SimpleFX id={trade_id}")
        print(f"  >>> {symbol} ENTRY: {direction} {lot:.2f}L @ SimpleFX id={trade_id}")
        if trade_id:
            tracked[trade_id] = {
                "symbol": symbol,
                "direction": direction,
...
def check_closed_positions(active_orders):
    closed_ids = set(tracked.keys()) - set(active_orders.keys())
...
def print_status(balance, positions):
    now = datetime.now(timezone.utc) + timedelta(hours=8)
    print(f"\n{'='*55}")
    print(f"  Paper Trader (SimpleFX)  |  {now.strftime('%H:%M')} Manila  |  Bal: ${balance:.2f}")
    print(f"{'='*55}")
    for p in positions:
        sym = p.get("symbol", "?")
        side = p.get("side", "?")
...
def run():
    print(f"\n{'='*55}")
    print(f"  M5 Paper Trader — SimpleFX Live Execution")
    print(f"  Pair: {SYMBOLS[0]}  |  Server: SimpleFX DEMO")
    print(f"{'='*55}\n")
    ...
    while True:
        try:
            ...
            active_orders, _ = broker.get_active_orders()
            ...
            print_status(balance, list(active_orders.values()))
            ...
            print(f"\n  Shutdown.")
            if trade_log:
                pd.DataFrame(trade_log).to_csv("logs/paper_trades.csv", index=False)
